...
- **Workflow:** Update the Japanese docs first using the `japanese-tech-writing` skill, then port the changes to the English pages. English is the default locale (`Website/docs/`); Japanese translations live in `Website/i18n/ja/docusaurus-plugin-content-docs/current/` and must mirror the same file paths, frontmatter (slugs), and explicit heading anchors (`{#id}`).
...
- **Avoid committing unintended changes to `Packages/*.json` and `ProjectSettings/*`:** These files are managed by Unity/VPM; only commit them when a PR explicitly intends to update Unity/VPM project configuration.
