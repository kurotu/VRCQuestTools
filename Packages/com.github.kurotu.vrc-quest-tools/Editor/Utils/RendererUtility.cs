...
namespace KRT.VRCQuestTools.Utils
{
    ...
    internal static class RendererUtility
    {
        ...
        internal static Mesh RemoveTransparentPart(Renderer renderer)
        {
            var mesh = GetSharedMesh(renderer);
            if (mesh == null)
            {
            ...
            var indicesToRemove = GetVertexIndicesToRemoveTransparentPart(renderer);
            ...
            // RemoveVerticesByIndices(newMesh, indicesToRemove);
            var newMesh = RemoveVertices(mesh, indicesToRemove);
            newMesh.name = mesh.name;
            return newMesh;
        }

        private static HashSet<int> GetVertexIndicesToRemoveTransparentPart(Renderer renderer)
        {
            var mesh = GetSharedMesh(renderer);
            var renderedTextures = renderer.sharedMaterials
            ...
            for (var index = 0; index < mesh.subMeshCount; index++)
            {
                ...
                var triangleGroups = triangles.Select((value, index) => (value, index))
                    ...
                    .ToArray();
                var texture = renderedTextures[index];
            ...
            return Enumerable.Range(0, mesh.vertexCount - 1).
        ...
        private static Mesh RemoveVertices(Mesh mesh, HashSet<int> indicesToRemove)
        {
            ...
            for (int i = 0; i < oldVertices.Length; i++)
            {
                if (!indicesToRemove.Contains(i))
                {
                    ...
                    if (oldNormals.Length == oldVertices.Length) newNormals.Add(oldNormals[i]);
                    if (oldTangents.Length == oldVertices.Length) newTangents.Add(oldTangents[i]);
                    if (oldColors.Length == oldVertices.Length) newColors.Add(oldColors[i]);
                    if (oldUV.Length == oldVertices.Length) newUV.Add(oldUV[i]);
                    if (oldUV2.Length == oldVertices.Length) newUV2.Add(oldUV2[i]);
                    if (oldUV3.Length == oldVertices.Length) newUV3.Add(oldUV3[i]);
                    if (oldUV4.Length == oldVertices.Length) newUV4.Add(oldUV4[i]);
                    if (oldBoneWeights.Length == oldVertices.Length) newBoneWeights.Add(oldBoneWeights[i]);
                }
            }
            ...
            if (newTangents.Count == newVertices.Count) newMesh.tangents = newTangents.ToArray();
            if (newColors.Count == newVertices.Count) newMesh.colors = newColors.ToArray();
            if (newUV.Count == newVertices.Count) newMesh.uv = newUV.ToArray();
            ...
            if (newBoneWeights.Count == newVertices.Count) newMesh.boneWeights = newBoneWeights.ToArray();
            ...
            for (int sub = 0; sub < mesh.subMeshCount; sub++)
            {
                ...
                for (int i = 0; i < oldTris.Length; i += 3)
                {
                    ...
                    if (indicesToRemove.Contains(i0) || indicesToRemove.Contains(i1) || indicesToRemove.Contains(i2)) continue;
