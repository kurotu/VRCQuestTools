...
namespace KRT.VRCQuestTools.Utils
{
    ...
    internal static class TextureOverrideUtility
    {
        ...
        internal static PlatformOverrideResult GetBestPlatformOverride(IEnumerable<Texture> sourceTextures)
        {
            ...
            foreach (var texture in sourceTextures)
            {
                ...
                // Check Android platform override
                var androidOverride = importer.GetPlatformTextureSettings("Android");
                if (androidOverride.overridden)
                {
                    ProcessPlatformOverride(androidOverride, ref result);
                }

                // Check iOS platform override  
                var iosOverride = importer.GetPlatformTextureSettings("iPhone");
                if (iosOverride.overridden)
                {
                    ProcessPlatformOverride(iosOverride, ref result);
                }
            }
