...
VRChat Creator Companion (VCC) でプロジェクトを作成し、アバターをセットアップしておいてください。

## VRCQuestTools をインストールする

1. [Add to VCC](https://kurotu.github.io/vpm-repos/vpm.html) のページを開き、リポジトリを VCC に追加します。
2. VCC でプロジェクトの「Manage Project」を開き、「VRCQuestTools」を追加します。

インストールが終わると、Unity のメニューバーに「Tools」→「VRCQuestTools」が表示されます。

:::tip 非破壊変換のための NDMF
プロジェクトに Non-Destructive Modular Framework (NDMF) パッケージがあると、元のアバターを変更せずにアップロード時に自動で変換できます。
NDMF は [bd_ 氏の VPM リポジトリ](https://vpm.nadena.dev/ja)から追加できます。
[Modular Avatar](https://modular-avatar.nadena.dev/ja) を導入している場合、NDMF はすでにプロジェクトに含まれています。
:::
...
iOS 版の VRChat 向けにもアバターをアップロードする場合は、iOS Build Support モジュールも必要です。
