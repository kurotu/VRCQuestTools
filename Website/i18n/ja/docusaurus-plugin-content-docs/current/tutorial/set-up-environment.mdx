...
Unity Hubを使わずにUnityをインストールした場合は、Unityのインストーラーを再度実行することでAndroid Build Supportを追加できます。

1. [Unity Download Archive](https://unity.com/releases/editor/archive)から使用しているUnityバージョンのインストーラーをダウンロードします。
    :::tip
    現在使用しているUnityと同じバージョンをダウンロードしてください。
    Unityのバージョンは、Unity Editorのメニューバーから**Help** > **About Unity**で確認できます。
    :::
2. Unityのインストーラーを実行します。
3. コンポーネント選択画面で**Android Build Support**を選択します。
    :::note
    **OpenJDK**を選択する必要はありません。
    **Android SDK & NDK Tools**も必須ではありませんが、アップロードせずにAndroid端末で「Build & Test」(ローカルテスト)をするために必要です。
    :::
