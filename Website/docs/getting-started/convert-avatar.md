...
"Conversion" may sound like a big deal, but the first time only takes a few button presses.
...
The settings may look overwhelming, but you can convert the avatar with the defaults.
...
1. Open the VRChat SDK control panel and select your avatar in the "Builder" tab.
2. Select Windows and Android in "Platform(s)".
3. Upload the avatar with the "Multi-Platform Build & Publish" button.

![Platform selection in the VRChat SDK control panel](/img/multi_platform_build.png)

The same avatar is uploaded to both PC and Android.
The automatic conversion happens only in the Android build.
VRChat can hold different avatar data for each platform: players on PC see the avatar before the conversion, while players on Android see the converted one.
...
The upload is done, but the converted avatar does not necessarily look the same as the PC version.
